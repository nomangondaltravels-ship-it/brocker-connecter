    ...
    async function editProperty(id) {
      ...
      document.getElementById('propertyPetsPolicy').value = property.petsAvailable ? 'allowed' : 'not_allowed';
    ...
    async function refreshMarketplaceItem(entityType, id, buttonCandidate = null) {
      await proceedRefreshMarketplaceItem(entityType, id, buttonCandidate);
    }
    ...
    function getMarketplaceListingForRecord(entityType, id) {
      const sourceType = entityType === 'lead' ? 'lead' : 'property';
      return (Array.isArray(state.sharedListings) ? state.sharedListings : []).find(item =>
        item.sourceType === sourceType && String(item.sourceId) === String(id)
      ) || null;
    }

    function getMarketplaceRefreshTimestamp(entityType, item) {
      const publicListing = getMarketplaceListingForRecord(entityType, item?.id);
      return publicListing?.marketplaceSortAt
        || publicListing?.marketplaceRefreshedAt
        || publicListing?.updatedAt
        || item?.marketplaceSortAt
        || item?.marketplaceRefreshedAt
        || item?.updatedAt
        || item?.createdAt
        || '';
    }
    ...
    function getMarketplaceRefreshState(entityType, item) {
      ...
      if (remainingMs > 0) {
        const waitLabel = formatMarketplaceRefreshWait(remainingMs);
        return {
          visible: true,
          disabled: true,
          label: `Refresh in ${waitLabel}`,
    ...
    function renderMarketplaceRefreshInlineButton(entityType, item, extraClass = '') {
      const refreshState = getMarketplaceRefreshState(entityType, item);
      if (!refreshState.visible) return '';
      return `
        <button class="btn btn-secondary btn-tiny ${extraClass}" type="button" ${refreshState.disabled ? 'disabled' : ''} title="${escapeHtml(refreshState.title)}" onclick="refreshMarketplaceItem('${entityType}', ${item.id}, this)">
          ${escapeHtml(refreshState.label)}
        </button>
      `;
    }

    function renderMarketplaceRefreshActionButton(entityType, item) {
      const refreshState = getMarketplaceRefreshState(entityType, item);
      if (!refreshState.visible) return '';
      return renderRecordActionButton({
        label: refreshState.label,
        icon: 'refresh',
        tone: 'secondary',
        onclick: `refreshMarketplaceItem('${entityType}', ${item.id}, this)`,
      ...
      return `
        <div class="record-action-bar">
          ...
          ${renderRecordActionButton({
            ...
            ${renderMarketplaceRefreshActionButton('lead', lead)}
          </div>
        `;
    }
      ...
      return `
        <div class="record-action-bar">
          ...
          ${renderRecordActionButton({
            ...
            ${renderMarketplaceRefreshActionButton('property', property)}
          </div>
        `;
    }
      ...
      return `
        ...
        <div class="detail-section">
          ...
          <div class="detail-grid">
            ...
            ${getPropertyPurpose(property.purpose) === 'monthly_rent' ? `<div class="detail-cell"><small>Amenities</small><strong>${escapeHtml(joinDisplayParts([property.gymAvailable ? 'Gym' : '', property.poolAvailable ? 'Pool' : '', property.parkingAvailable ? 'Parking' : '']) || '--')}</strong></div>` : ''}
            ${getPropertyPurpose(property.purpose) === 'monthly_rent' ? `<div class="detail-cell"><small>Pets</small><strong>${escapeHtml(property.petsAvailable ? 'Pets Allowed' : 'Pets Not Allowed')}</strong></div>` : ''}
    ...
    function buildBulkListingShareMessage(properties = getSelectedBulkListingShareProperties(), includeContact = state.bulkListingShareIncludeContact) {
      ...
      items.forEach((property, index) => {
        ...
        const statusParts = [
          ...
          getPropertyPurpose(property.purpose) === 'monthly_rent' ? (property.petsAvailable ? 'Pets allowed' : 'Pets not allowed') : '',
